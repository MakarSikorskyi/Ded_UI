...
systemFrame:SetPoint("RIGHT", -310,0)
systemFrame:SetSize(180, 16)
---------------------------------------------------------------------
...
local addoncompare = function(a, b)
 ...
 if not cfg.system.showTooltip then return end
	...
	GameTooltip:AddLine(cfg.TooltipTitleText("Система"))
	GameTooltip:AddLine(" ")
	---------------------------------------------------
...
local function updatePerformanceText()
	...
	pingFrame:SetSize(pingText:GetStringWidth()+20, 16)
	fpsText:SetText(fps.."fps")
	fpsFrame:SetSize(fpsText:GetStringWidth()+20, 16)
	-- systemFrame:SetSize(pingText:GetStringWidth()+20+fpsText:GetStringWidth()+20, 16)
	if onHover then
		systemBarOnEnter()
	end
end
